namespace :scrape do
  ...
  task us_house: :environment do
    ...
    members.each do |member|
      ...
      representative = Representative.new(name: name, party: party, district: district, img: "?", profession: "Member of the US House of Representatives", rating:"?", url: url)
      representative.save!
  ...
  task us_senate: :environment do
    ...
    senator.each do |senator|
      url = senator.css("a").attr("href")
      name = senator.css("a").text.sub("Senator ","")
      name = name.split(", ")[1] + name.split(", ")[0]
      party = senator.css(".result-item")[1].css("span")[0]
